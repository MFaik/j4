...
public class RoomManager : MonoBehaviour
{
    ...
    void EndSpin()
    {
        ...
        Room1.transform.DORotate(new Vector3(0,0,0),.2f).SetDelay(.1f).OnComplete(EnablePlayer).SetDelay(.2f);
        Room2.transform.DORotate(new Vector3(0,0,0),.2f).SetDelay(.1f);
    }
    void EnablePlayer()
    {
        isPlaying = false;
